...
public final class CategoryDAO_Impl implements CategoryDAO {
  ...
  public CategoryDAO_Impl(@NonNull final RoomDatabase __db) {
    ...
    this.__insertAdapterOfCategory = new EntityInsertAdapter<Category>() {
      ...
      protected String createQuery() {
        return "INSERT OR ABORT INTO `categories` (`id`,`userId`,`name`,`icon`,`color`,`type`,`system`,`createdAt`,`updatedAt`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?)";
      }
  ...
  public LiveData<List<Category>> getCategoriesByType(final String type) {
    final String _sql = "SELECT * FROM categories WHERE type = ?";
    return __db.getInvalidationTracker().createLiveData(new String[] {"categories"}, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
